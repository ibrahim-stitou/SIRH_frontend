...
export default function DepartementsParametresPage() {
  ...
  const [_tableInstance, setTableInstance] = useState<Partial<
    UseTableReturn<DepartementSettingRow>
  > | null>(null);
  ...
  const onAskDelete = (row: DepartementSettingRow) =>
    setConfirmDeleteId(row.id);
  const onConfirmDelete = async () => {
    if (!confirmDeleteId) return;
    try {
      await apiClient.delete(
        apiRoutes.admin.parametres.departements.delete(confirmDeleteId)
      );
  ...
  const onToggleActive = useCallback(
    async (row: DepartementSettingRow) => {
      try {
        ...
        <div className='mb-2 flex items-center justify-between'>
          <div>
            ...
            <p className='text-muted-foreground text-sm'>
              Gérez les codes, libellés et statut d&lsquo;activité des
              départements
            </p>
          </div>
        ...
        <Dialog open={showAddModal} onOpenChange={setShowAddModal}>
          <DialogContent>
            ...
            <DialogFooter>
              ...
              <Button
                onClick={handleAdd}
                disabled={addLoading || !newCode.trim() || !newLibelle.trim()}
              >
