...
const DOMAINES: string[] = [
  'Informatique','Finance','RH','Commercial','Production','Qualite','Logistique','Marketing','Juridique','Achat'
];

export default function MetiersParametresPage() {
  const [_tableInstance, setTableInstance] = useState<Partial<UseTableReturn<MetierSettingRow>> | null>(null);
  ...
  const onConfirmDelete = async () => {
    ...
    try {
      await apiClient.delete(apiRoutes.admin.parametres.metiers.delete(confirmDeleteId));
  ...
  const onToggleActive = useCallback(async (row: MetierSettingRow) => {
    try {
    ...
    <PageContainer scrollable={false}>
      <div className='flex flex-col gap-4 w-full'>
        <div className='mb-2 flex items-center justify-between'>
          <div>
            <h1 className='text-2xl font-semibold tracking-tight'>Paramètres — Métiers</h1>
            <p className='text-muted-foreground text-sm'>Gérez les codes, libellés, domaine et statut d&apos;activité des métiers</p>
          </div>
          ...
          <DialogContent>
            ...
            <div className='grid gap-4'>
              <div className='grid gap-2'>
                ...
                <Input id='code' value={newCode} onChange={(e) => setNewCode(e.target.value)} placeholder='Ex: MET-DEV' />
              </div>
              <div className='grid gap-2'>
            ...
            <DialogFooter>
              ...
              <Button onClick={handleAdd} disabled={addLoading || !newCode.trim() || !newLibelle.trim() || !newDomaine}>
