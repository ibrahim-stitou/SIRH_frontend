...
interface DepartementSettingRow { id: number | string; code: string; libelle: string; is_active: boolean; }

export default function PostesParametresPage() {
  const [_tableInstance, setTableInstance] = useState<Partial<UseTableReturn<PosteSettingRow>> | null>(null);
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        const res = await apiClient.get(apiRoutes.admin.parametres.departements.list);
  ...
  const onConfirmDelete = async () => {
    ...
    try {
      await apiClient.delete(apiRoutes.admin.parametres.postes.delete(confirmDeleteId));
  ...
  const onToggleActive = useCallback(async (row: PosteSettingRow) => {
    try {
  ...
  const handleAdd = async () => {
    ...
    try {
      await apiClient.post(apiRoutes.admin.parametres.postes.create, {
        ...
        departement_id: isNaN(+newDepartementId) ? newDepartementId : +newDepartementId,
        is_active: newActive
      });
  ...
  const columns: CustomTableColumn<PosteSettingRow>[] = useMemo(() => [
    ...
    { data: 'libelle', label: 'Libellé', sortable: true },
    {
      ...
      label: 'Département',
      sortable: true,
      render: (v: number | string) => departementLabelById.get(String(v)) || String(v)
    },
    {
      data: 'is_active',
      label: 'Statut',
    ...
    <PageContainer scrollable={false}>
      <div className='flex flex-col gap-4 w-full'>
        <div className='mb-2 flex items-center justify-between'>
          <div>
            <h1 className='text-2xl font-semibold tracking-tight'>Paramètres — Postes</h1>
            <p className='text-muted-foreground text-sm'>Gérez les codes, libellés, département et statut d&apos;activité des postes</p>
          </div>
          ...
          <DialogContent>
            ...
            <div className='grid gap-4'>
              <div className='grid gap-2'>
                ...
                <Input id='code' value={newCode} onChange={(e) => setNewCode(e.target.value)} placeholder='Ex: POST-DEV' />
              </div>
              <div className='grid gap-2'>
                <Label htmlFor='libelle'>Libellé</Label>
                <Input id='libelle' value={newLibelle} onChange={(e) => setNewLibelle(e.target.value)} placeholder='Ex: Développeur' />
              </div>
              <div className='grid gap-2'>
                <Label>Département</Label>
                <Select value={newDepartementId} onValueChange={setNewDepartementId}>
                  <SelectTrigger>
                    <SelectValue placeholder='Choisir un département' />
                  </SelectTrigger>
                  <SelectContent>
                    {departements.map((d) => (
                      <SelectItem key={d.id} value={String(d.id)} disabled={!d.is_active}>
                        {d.libelle}
                      </SelectItem>
                    ))}
                  </SelectContent>
            ...
            <DialogFooter>
              ...
              <Button onClick={handleAdd} disabled={addLoading || !newCode.trim() || !newLibelle.trim() || !newDepartementId}>
