...
export default function EmploisParametresPage() {
  const [_tableInstance, setTableInstance] = useState<Partial<UseTableReturn<EmploiSettingRow>> | null>(null);
  ...
  const onConfirmDelete = async () => {
    ...
    try {
      await apiClient.delete(apiRoutes.admin.parametres.emplois.delete(confirmDeleteId));
  ...
  const onToggleActive = useCallback(async (row: EmploiSettingRow) => {
    try {
    ...
    <PageContainer scrollable={false}>
      <div className='flex flex-col gap-4 w-full'>
        <div className='mb-2 flex items-center justify-between'>
          <div>
            <h1 className='text-2xl font-semibold tracking-tight'>Paramètres — Emplois</h1>
            <p className='text-muted-foreground text-sm'>Gérez les codes, libellés, type de contrat et statut d&apos;activité des emplois</p>
          </div>
          ...
          <DialogContent>
            ...
            <div className='grid gap-4'>
              <div className='grid gap-2'>
                ...
                <Input id='code' value={newCode} onChange={(e) => setNewCode(e.target.value)} placeholder='Ex: EMP-CDI-DEV' />
              </div>
              <div className='grid gap-2'>
                <Label htmlFor='libelle'>Libellé</Label>
                <Input id='libelle' value={newLibelle} onChange={(e) => setNewLibelle(e.target.value)} placeholder='Ex: Développeur CDI' />
              </div>
              <div className='grid gap-2'>
                <Label>Type de contrat</Label>
                <Select value={newTypeContrat} onValueChange={setNewTypeContrat}>
                  <SelectTrigger>
            ...
            <DialogFooter>
              ...
              <Button onClick={handleAdd} disabled={addLoading || !newCode.trim() || !newLibelle.trim() || !newTypeContrat}>
