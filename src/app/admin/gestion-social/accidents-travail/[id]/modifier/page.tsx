...
export default function ModifierAccidentTravailPage() {
  ...
  useEffect(() => {
    const loadAccident = async () => {
      try {
        ...
        const data: AccidentTravail = response.data.data;
        setAccident(data);

        // Populate form
        form.reset({
          employeId: String(data.employeId),
          dateHeureAccident: data.dateHeureAccident.substring(0, 16), // Format for datetime-local
  ...
  const onSubmit = async (data: AccidentFormValues) => {
    ...
    try {
      ...
      const payload = {
        ...
        statut: data.statut,
        modifiePar: 'RH Manager'
      };

      await apiClient.put(apiRoutes.admin.accidentsTravail.update(id), payload);
      toast.success('Accident modifié avec succès');
      router.push('/admin/gestion-social/accidents-travail');
    } catch (error: any) {
      console.error('Erreur:', error);
      toast.error(
                ...
                <FormField
                  ...
                  render={() => (
                    <FormItem>
                      ...
                      />
                      <FormDescription>
                        Enregistrer en brouillon ou déclarer
                      </FormDescription>
                      <FormMessage />
                    </FormItem>
                  )}
