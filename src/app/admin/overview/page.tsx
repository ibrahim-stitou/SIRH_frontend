...
export default function OverviewDashboard() {
  ...
  const payrollByDept = useMemo(() => {
    ...
    return Object.entries(map)
      .map(([deptId, net]) => ({
        dept:
          data.departments.find((d: Department) => d.id === Number(deptId))
            ?.name || `Dept ${deptId}`,
        net
      }))
  ...
  const leaveStatusData = useMemo(() => {
    ...
    data.leaves.forEach(
      (l: Leave) => (counts[l.status] = (counts[l.status] || 0) + 1)
    );
  ...
  const publishedPayslips = payslips.filter(
    (p: Payslip) => p.status === 'published'
  );
  const totalNetPayroll = publishedPayslips.reduce(
    (s: number, p: Payslip) => s + (p.net || 0),
    0
  );
  const averageEvaluation = evaluations.length
    ? (
        evaluations.reduce(
          (s: number, ev: Evaluation) => s + (ev.score || 0),
          0
        ) / evaluations.length
      ).toFixed(1)
    : 'N/A';
  ...
  return (
    <PageContainer scrollable={true}>
      <div className='min-h-screen w-full bg-gradient-to-br from-slate-50 via-blue-50 to-indigo-50 p-0 dark:from-slate-950 dark:via-slate-900 dark:to-indigo-950'>
        <div className='mx-auto sm:p-6 lg:p-8'>
          ...
          <div className='mb-8 grid grid-cols-1 gap-6 xl:grid-cols-3'>
            ...
            <ChartBox
              ...
              subtitle={`Total: MAD${totalNetPayroll.toLocaleString()}`}
            >
              <ResponsiveContainer width='100%' height={240}>
...
function DashboardSkeleton() {
  return (
    <PageContainer scrollable={true}>
      {' '}
      <div className='min-h-screen w-full bg-gradient-to-br from-slate-50 via-blue-50 to-indigo-50 p-0 dark:from-slate-950 dark:via-slate-900 dark:to-indigo-950'>
        <div className='mx-auto animate-pulse p-4 sm:p-6 lg:p-8'>
          {/* Metrics skeleton */}
          <div className='mb-8 grid grid-cols-1 gap-6 sm:grid-cols-2 lg:grid-cols-4'>
            {[1, 2, 3, 4].map((i) => (
              <div
                key={i}
                className='h-32 rounded-2xl bg-white/50 dark:bg-slate-900/50'
              />
            ))}
          </div>
          ...
          <div className='mb-8 grid grid-cols-1 gap-6 xl:grid-cols-3'>
            {[1, 2, 3].map((i) => (
              <div
                key={i}
                className='h-80 rounded-2xl bg-white/50 dark:bg-slate-900/50'
              />
            ))}
          </div>
...
function MetricCard({
  ...
  return (
    <div className='group relative overflow-hidden rounded-2xl border border-white/20 bg-white/80 p-6 shadow-lg backdrop-blur-xl transition-all duration-300 hover:-translate-y-1 hover:shadow-2xl dark:border-slate-700/50 dark:bg-slate-900/80'>
      <div
      ...
      <div className='relative flex items-start justify-between'>
        <div className='flex-1'>
          <div
            className={`inline-flex rounded-xl bg-gradient-to-br p-3 ${gradient} mb-4 shadow-lg`}
          >
            <div className='text-white'>{icon}</div>
          </div>
          <div className='mb-2 text-sm font-medium tracking-wide text-slate-600 uppercase dark:text-slate-400'>
            {label}
          </div>
        ...
        {trend && trend.length === 2 && (
          <div
            className={`flex flex-col items-end ${trend[1] >= trend[0] ? 'text-emerald-600' : 'text-rose-600'}`}
          >
            <span className='text-2xl'>{trend[1] >= trend[0] ? '↑' : '↓'}</span>
            <span className='text-sm font-semibold'>
              {Math.abs(trend[1] - trend[0])}
            </span>
          </div>
        )}
      </div>
...
function ChartBox({
  ...
  return (
    <div className='rounded-2xl border border-white/20 bg-white/80 p-6 shadow-lg backdrop-blur-xl transition-shadow duration-300 hover:shadow-xl dark:border-slate-700/50 dark:bg-slate-900/80'>
      <div className='mb-6'>
...
function ListBox({
  ...
  return (
    <div className='rounded-2xl border border-white/20 bg-white/80 p-6 shadow-lg backdrop-blur-xl dark:border-slate-700/50 dark:bg-slate-900/80'>
      ...
      <div className='space-y-2'>
        {items.map((item) => (
          <div
            key={item.id}
            className='flex items-center justify-between rounded-xl bg-slate-50/50 p-3 transition-colors duration-200 hover:bg-slate-100/80 dark:bg-slate-800/50 dark:hover:bg-slate-800/80'
          >
