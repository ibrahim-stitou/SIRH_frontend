module.exports = function registerSettingsPostesRoutes(server, db) {
  ...
  server.get('/settings/postes/:id', (req, res) => {
    ...
    const row = (db.get('settingsPostes').value() || []).find((r) => String(r.id) === String(id));
  ...
  server.post('/settings/postes', (req, res) => {
    ...
    if (!code || !libelle || !departement_id)
      return res.status(400).json({ status: 'error', message: 'Code, libellé et département requis', data: null });

    // Optional: prevent duplicate code
    const existsCode = (db.get('settingsPostes').value() || []).some((r) => String(r.code).toLowerCase() === code.toLowerCase());
    ...
    const depExists = (db.get('settingsDepartements').value() || []).some((d) => String(d.id) === String(departement_id));
    if (!depExists) return res.status(400).json({ status: 'error', message: 'Département inconnu', data: null });
  ...
  server.put('/settings/postes/:id', (req, res) => {
    ...
    let next = { ...exists, ...req.body, id, updated_at: new Date().toISOString() };
    if (next.departement_id) {
      const depExists = (db.get('settingsDepartements').value() || []).some((d) => String(d.id) === String(next.departement_id));
      if (!depExists) return res.status(400).json({ status: 'error', message: 'Département inconnu', data: null });
    }
  ...
  server.patch('/settings/postes/:id/activate', (req, res) => {
    ...
    const exists = db.get('settingsPostes').find({ id }).value();
    if (!exists) return res.status(404).json({ status: 'error', message: 'Introuvable', data: null });
    db.get('settingsPostes').find({ id }).assign({ is_active: true, updated_at: new Date().toISOString() }).write();
    const updated = db.get('settingsPostes').find({ id }).value();
  ...
  server.patch('/settings/postes/:id/deactivate', (req, res) => {
    ...
    const exists = db.get('settingsPostes').find({ id }).value();
    if (!exists) return res.status(404).json({ status: 'error', message: 'Introuvable', data: null });
    db.get('settingsPostes').find({ id }).assign({ is_active: false, updated_at: new Date().toISOString() }).write();
    const updated = db.get('settingsPostes').find({ id }).value();
