module.exports = function registerSettingsMetiersRoutes(server, db) {
  ...
  server.get('/settings/metiers/:id', (req, res) => {
    ...
    const row = (db.get('settingsMetiers').value() || []).find((r) => String(r.id) === String(id));
  ...
  server.post('/settings/metiers', (req, res) => {
    ...
    if (!code || !libelle || !domaine)
      return res.status(400).json({ status: 'error', message: 'Code, libellé et domaine requis', data: null });

    // Optional: prevent duplicate code
    const existsCode = (db.get('settingsMetiers').value() || []).some((r) => String(r.code).toLowerCase() === code.toLowerCase());
  ...
  server.patch('/settings/metiers/:id/activate', (req, res) => {
    ...
    const exists = db.get('settingsMetiers').find({ id }).value();
    if (!exists) return res.status(404).json({ status: 'error', message: 'Introuvable', data: null });
    db.get('settingsMetiers').find({ id }).assign({ is_active: true, updated_at: new Date().toISOString() }).write();
    const updated = db.get('settingsMetiers').find({ id }).value();
  ...
  server.patch('/settings/metiers/:id/deactivate', (req, res) => {
    ...
    const exists = db.get('settingsMetiers').find({ id }).value();
    if (!exists) return res.status(404).json({ status: 'error', message: 'Introuvable', data: null });
    db.get('settingsMetiers').find({ id }).assign({ is_active: false, updated_at: new Date().toISOString() }).write();
    const updated = db.get('settingsMetiers').find({ id }).value();
