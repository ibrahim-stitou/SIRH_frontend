module.exports = function registerSettingsConditionsContratRoutes(server, db) {
  // List
  server.get('/settings/conditions-contrat', (req, res) => {
    const rows = db.get('settingsConditionsContrat').value() || [];
    return res.json({ status: 'success', message: 'Récupération réussie', data: rows });
  });

  // Show by ID
  server.get('/settings/conditions-contrat/:id', (req, res) => {
    const id = isNaN(+req.params.id) ? req.params.id : +req.params.id;
    const row = (db.get('settingsConditionsContrat').value() || []).find((r) => String(r.id) === String(id));
  ...
  // Create
  server.post('/settings/conditions-contrat', (req, res) => {
    const p = req.body || {};
    ...
    const exists = (db.get('settingsConditionsContrat').value() || []).some((r) => String(r.name).toLowerCase() === name.toLowerCase());
    ...
    db.get('settingsConditionsContrat').push(row).write();
    return res.status(201).json({ status: 'success', message: 'Création réussie', data: row });
  });

  // Update
  server.put('/settings/conditions-contrat/:id', (req, res) => {
    const id = isNaN(+req.params.id) ? req.params.id : +req.params.id;
    const exists = db.get('settingsConditionsContrat').find({ id }).value();
    ...
    delete next.value;
    db.get('settingsConditionsContrat').find({ id }).assign(next).write();
    const updated = db.get('settingsConditionsContrat').find({ id }).value();
    return res.json({ status: 'success', message: 'Mise à jour réussie', data: updated });
  });

  // Delete
  server.delete('/settings/conditions-contrat/:id', (req, res) => {
    const id = isNaN(+req.params.id) ? req.params.id : +req.params.id;
    const exists = db.get('settingsConditionsContrat').find({ id }).value();
    if (!exists) return res.status(404).json({ status: 'error', message: 'Introuvable', data: null });
    db.get('settingsConditionsContrat').remove({ id }).write();
