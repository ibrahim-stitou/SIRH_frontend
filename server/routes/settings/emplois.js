...
module.exports = function registerSettingsEmploisRoutes(server, db) {
  const isValidContractType = (t) => {
    ...
    const s = String(t);
    const allowed =
      ContractTypes && Array.isArray(ContractTypes)
        ? ContractTypes
        : [
            'CDI',
            'CDD',
  ...
  server.get('/settings/emplois/:id', (req, res) => {
    ...
    const row = (db.get('settingsEmplois').value() || []).find(
  ...
  server.post('/settings/emplois', (req, res) => {
    ...
    if (!code || !libelle || !type_contrat)
      return res
        ...
        .json({
          status: 'error',
          message: 'Code, libellé et type de contrat requis',
          data: null
        });
    ...
    const existsCode = (db.get('settingsEmplois').value() || []).some(
